/**
 * Month-to-date merged PRs against the standing monthly target: a hero figure
 * and meter on the left, a burn-up of the month on the right.
 */
...
export function MonthlyProgressCard({
  ...
  if (error) return <ErrorBanner message={`Monthly progress: ${error}`} />;

  if (!progress) {
    if (!isLoading) return null;
    return (
      <div className="bg-surface rounded-xl shadow-(--shadow-skeuo-card) border border-(--color-border) p-5 h-[17rem] animate-pulse" />
    );
  }
  ...
  return (
    <section
      aria-label="Monthly PR target"
      className={`bg-surface rounded-xl shadow-(--shadow-skeuo-card) hover:shadow-(--shadow-skeuo-card-hover) border border-(--color-border) p-5 transition-all duration-300 ${
        isLoading ? "opacity-90" : ""
      }`}
    >
      <div className="flex items-center justify-between gap-4 pb-4">
        <h2 className="flex items-center gap-2 text-base font-semibold text-(--color-text) min-w-0">
          <Target size={20} weight="regular" className="text-primary-500 shrink-0" />
          <span className="truncate">Monthly target</span>
          <span className="text-text-muted font-normal truncate">
            {progress.label}
          </span>
        </h2>
        <span
          className={`flex items-center gap-1.5 shrink-0 px-2.5 py-1 rounded-full border text-xs font-medium ${status.pill}`}
        >
      ...
      <div className="pt-4 border-t border-(--color-border) grid gap-6 lg:grid-cols-[minmax(0,15rem)_minmax(0,1fr)_auto]">
        <div className="space-y-4 min-w-0">
          <div>
            <div className="flex items-baseline gap-2">
              ...
              <span
                className={`text-5xl font-semibold leading-none ${status.accent}`}
              >
                {formatNumber(progress.merged)}
              </span>
              <span className="text-sm text-text-muted">
                of {formatNumber(pace.target)} merged
              </span>
            </div>

            <div className="mt-3">
              {/* Just fill vs target. Pace belongs to the chart's dashed line
                  and the "Pace to date" row — a marker here read as a defect. */}
              <div className="h-2.5 rounded-full bg-primary-500/15 overflow-hidden">
                <div
        ...
        <div className="min-w-0">
          ...
          >
            <ChartContainer
              config={CHART_CONFIG}
              className="h-full w-full min-h-[256px] aspect-auto"
            >
              <ComposedChart
                data={rows}
              ...
              >
                <defs>
                  <linearGradient id="monthlyActualFill" x1="0" y1="0" x2="0" y2="1">
                ...
                <XAxis
                  ...
                  tickMargin={8}
                  interval={0}
                  ticks={X_TICKS.filter((t) => t <= progress.daysInMonth).map(String)}
                />
                <YAxis
                  domain={[0, yMax]}
                  tickLine={false}
