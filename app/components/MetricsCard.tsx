...
export function MetricsCard({
  ...
  return (
    <div className="bg-[var(--color-surface)] rounded-xl shadow-[var(--shadow-skeuo-card)] hover:shadow-[var(--shadow-skeuo-card-hover)] border border-[var(--color-border)] p-5 transition-all duration-300 xl:h-full xl:flex xl:flex-col xl:min-h-0">
          ...
          >
            <div className="overflow-hidden min-h-0">
              ...
              >
                {payload!.github.merged_prs.length > 0 && (
                  <div>
                    <h3 className="font-medium text-[var(--color-text-muted)] mb-2">
                      PRs merged
                    </h3>
                ...
                {payload!.linear.completed_issues.length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-1">
                      {(payload!.linear.completed_issues as LinearIssue[]).map(
                        (i, idx) => (
                          <li key={idx}>
                            {i.url ? (
                              <a
                ...
                {payload!.linear.worked_on_issues.length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-1">
                      {(payload!.linear.worked_on_issues as LinearIssue[]).map(
                        (i, idx) => (
                          <li key={idx}>
                            {i.url ? (
                              <a
                ...
                {(payload!.linear.created_issues?.length ?? 0) > 0 && (
                  <div>
                    ...
                    <ul className="space-y-1">
                      {(
                        (payload!.linear.created_issues ?? []) as LinearIssue[]
                      ).map((i, idx) => (
                        <li key={idx}>
                          {i.url ? (
                            <a
